    ...
    await act(async () => {
      window.dispatchEvent(
        new CustomEvent(CHAT_PREFILL_EVENT, {
          detail: { text: "Show my tiny.place identity status.", select: true },
        }),
      );
    });

    expect(input.value).toBe("Show my tiny.place identity status.");
    ...
    try {
      ...
      act(() => {
        window.dispatchEvent(
          new CustomEvent(CHAT_PREFILL_EVENT, {
            detail: {
              text: "Show my tiny.place identity status.",
              select: true,
            },
          }),
        );
